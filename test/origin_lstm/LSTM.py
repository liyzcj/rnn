...
def run_epoch(session, model, eval_op=None, verbose=False):
    ...
    # for step in range(3):
        ...
        # vals = session.run(fetches, feed_dict, options=run_options, run_metadata=run_metadata)
        cost = vals["cost"]
        ...
        # with open('timeline_LSTM_step_%d.json' % step, 'w') as f:
...
with tf.Graph().as_default():
    ...
    sv = tf.train.Supervisor(logdir="test/origin_lstm/logs/", init_op=init)  # logdir��������checkpoint��summary
    saver=sv.saver    
